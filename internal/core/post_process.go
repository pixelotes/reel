...
func NewPostProcessor(cfg *config.Config, logger *utils.Logger) *PostProcessor {
...
func (pp *PostProcessor) ProcessDownload(media models.Media, torrentStatus torrent.TorrentStatus) {
	pp.logger.Info("Starting post-processing for:", media.Title)

	destinationPath := pp.createDestinationFolder(&media)
	if destinationPath == "" {
...
func (pp *PostProcessor) createDestinationFolder(media *models.Media) string {
	pp.logger.Info("[DUMMY] Creating destination folder for:", media.Title)
	// In the future, this will create a path like "/media/movies/Movie Title (2023)"
	return "/dummy/destination/path"
}
